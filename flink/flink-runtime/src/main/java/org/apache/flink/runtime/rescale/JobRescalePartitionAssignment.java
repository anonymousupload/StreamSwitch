...
public class JobRescalePartitionAssignment {
	...
	public int getOldNumOpenedSubtask() {
		return oldRescalePA.getNumOpenedSubtask();
	}
	...
	public Map<Integer, List<Integer>> getOldPartitionAssignment() {
		return oldRescalePA.getPartitionAssignment();
	}

	public Map<String, List<String>> getStrPartitionAssignment() {
		return generateStringMap(getPartitionAssignment());
	}

	public JobRescalePartitionAssignment getOldJobRescalePartitionAssignment() {
		return oldRescalePA;
	}
	...
	// map of integer -> map of string
	private static Map<String, List<String>> generateStringMap(
		Map<Integer, List<Integer>> partitionAssignment) {

		Map<String, List<String>> mapping = new HashMap<>();
		for (Integer subTaskIndexStr : partitionAssignment.keySet()) {
			String subTaskIndex = String.valueOf(subTaskIndexStr);
			List<String> partitions = new ArrayList<>();

			for (Integer partitionStr : partitionAssignment.get(subTaskIndexStr)) {
				partitions.add(String.valueOf(partitionStr));
			}
			mapping.put(subTaskIndex, partitions);
		}
